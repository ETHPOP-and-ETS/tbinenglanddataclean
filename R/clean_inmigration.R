...
#' A Function to Clean and Merge Observed and Projected Inmigration in England
#' 
...
#'
clean_inmigration <- function(inmigration_path,
                              return = TRUE,
                              save = TRUE,
                              save_name = "uk_births",
                              save_path,
  ...
  migration_person <- read_excel(inmigration_path,
                                 sheet = "PERSONS",
                                 range = cell_rows(c(n_toprow,n_bottomrow)))
  migration_male <- read_excel(inmigration_path,
                               sheet = "MALES",
                               range = cell_rows(c(n_toprow,n_bottomrow)))
  migration_female <- read_excel(inmigration_path,
                                 sheet = "FEMALES",
                                 range = cell_rows(c(n_toprow,n_bottomrow)))
  
  row_names <- migration_person[, "...1"]
  migration_row <- which(row_names == "International migration inflows" &
                       !is.na(row_names))
  ...
  proj_migration <-
    ...
    pivot_longer(cols = `2018`:`2117`,
                 names_to = "year",
                 values_to = "inflow") %>%
    mutate(inflow = as.numeric(inflow) * 1000,
